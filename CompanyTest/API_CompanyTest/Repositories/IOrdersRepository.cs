...
namespace API_CompanyTest.Repositories
{
    ...
    {
        Task<DTO_Result> CreateOrderAsync(Order order);
        Task<DTO_Result> DeleteOrderAsync(Order order);
    ...
    {
        public async Task<DTO_Result> CreateOrderAsync(Order order)
        {
            try
            {
                context.Orders.Add(order);
                var result = await context.SaveChangesAsync();
                return new DTO_Result(result > 0, result > 0 ? "Order created successfully." : "Failed to create order.");
            }
            catch (Exception ex)
            {
                return new DTO_Result(false, $"Error creating order: {ex.Message}");
            }
        }
        public async Task<DTO_Result> DeleteOrderAsync(Order order)
        {
            context.Remove(order);
            var result = await context.SaveChangesAsync();
            return new DTO_Result(result > 0, result > 0 ? "Order deleted successfully." : "Failed to delete order.");
        ...
        public async Task<DTO_Result> UpdateOrderAsync(Guid id, DTO_Order order_dto)
        {
            ...
            var result = await context.SaveChangesAsync();
            return new DTO_Result(result > 0, result > 0 ? "Order updated successfully." : "Failed to update order.");
        }
